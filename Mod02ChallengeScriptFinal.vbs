...
Sub stockAnalysis():
    ...
    Dim percentageChange As Double ' variable that holds the percent change for each stock in a sheet
    Dim summaryTableRow As Long ' variable that holds the row of the summary table row
    ...
    For Each ws In Worksheets
        ...
        For row = 2 To rowCount
            ...
            If ws.Cells(row + 1, 1).Value <> ws.Cells(row, 1).Value Then
                ...
                ' check to see if the total stock volume is 0
                If total = 0 Then
                    ...
                    ws.Range("J" & 2 + summaryTableRow).Value = 0 'prints a 0 in the column J (quarterly Change)
               ...
               Else
                    ...
                    ' calculate the percentage change (quarterly change / first open)
                    percentChange = quarterlyChange / ws.Cells(startValue, 3).Value
                    
                    ' print the results in the summary table section (Columns I - L)
                    ws.Range("I" & 2 + summaryTableRow).Value = ws.Cells(row, 1).Value  ' prints the ticker value from column A
                    ws.Range("J" & 2 + summaryTableRow).Value = quarterlyChange  'prints a value in the column J (quarterly Change)
                    ws.Range("K" & 2 + summaryTableRow).Value = percentChange ' prints a value in column K (% Change)
                    ws.Range("L" & 2 + summaryTableRow).Value = total  'Prints a a value in column L (total stock volume)
                    ...
                    quarterlyChange = 0 ' resets the quarterly change for the ticker
        ...
        ' use Match () to find the row numbers of the ticker names associated with the greatest % increase and decrease, the find the greatest total stock vol
        ...
        For s = 0 To summaryTableRow
            ws.Range("J" & 2 + s).NumberFormat = "0.00" ' formats the Quartely Change
        ...
        'Auto fit the info across all columns
